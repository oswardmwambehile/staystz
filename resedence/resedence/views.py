...
def residence_booking(request, pk):
    property = get_object_or_404(ResidenceProperty, pk=pk)

    if request.method == "POST":
        form = ResidenceBookingForm(request.POST)
        if form.is_valid():
            rent_duration = int(form.cleaned_data['rent_duration'])
            phone = form.cleaned_data['phone_number']

            price_per_month = property.residencepropertypricing.base_price
            total_price = rent_duration * price_per_month

            ResidenceBooking.objects.create(
                user=request.user,
                property=property,
            ...
            return redirect("booking_success")  # create this page

    else:
        form = ResidenceBookingForm()

    return render(request, "customer/resedence-booking.html", {
        "property": property,
        "form": form
    })
